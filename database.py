...
class Database(object):
    ...
    def export_db(self, max_depth: int = 4, schema: str = 'd1464', base_dir: str = 'assets/exports'):
        with self.engine.connect() as con:
            ...
            if cleanup:
                ...
                con.execute('''
            delete from public.algorithms where id not in (
                select a.id from public.algorithms a
                join public.datasets d on a.input_dataset = d.id
            ...
            from (
                select id, unnest(replacement) as replacement from (
                    select min(id) as id, count(*), array_agg(id) as replacement
                    from datasets d
